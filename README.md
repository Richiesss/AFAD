...
| FedGen Only | 57.61% | 57.43% | 2.3869 | 2,450s |
...
> **Phase 2 の主な知見**:
>
> 1. **FedGen Only は集約なしで大幅に精度低下** (57.61% vs HeteroFL 70.91%, −13.3pt)。
...
> 5. **損失値**: FedGen Only は loss=2.39 と高止まりし、収束が不十分。HeteroFL/AFAD は loss≈1.4 で安定収束
>
...
| 計算コスト | FedGen Only は最遅 (2,450s)。集約なしで KD のみは非効率 |
...
- **HeteroFL 集約は不可欠**: 同族間の重み平均がない FedGen Only は 57.6% に留まり、KD だけでは集約を代替できない
