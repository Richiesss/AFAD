...
10 seeds の paired t-test において、AFAD Hybrid は **いずれのベースラインに対しても p < 0.05 の有意差を示していない**（vs HeteroFL: p=0.053、vs FedGen: p=0.423）。したがって、現データのみでは「AFAD が既存手法を精度面で有意に上回る」とは主張できない。

| 側面 | 評価 |
|------|------|
| 平均精度 | 全方式中最高 (72.11%) だが、FedGen との差は +0.16pt と僅差 |
| 安定性 | 分散最小 (std=2.61%)、AFAD ≥ HeteroFL が 10/10 seeds で成立 |
| 統計的有意性 | いずれの比較も p < 0.05 を未達。エビデンス不十分 |
| 計算コスト | HeteroFL + FedGen の統合により、HeteroFL 単体比で追加の計算コストが発生 |
| IID 環境 | Phase 1 では 3 方式ほぼ同等。AFAD 固有の優位性は見られない |

### 限界と今後の課題

現在の実験設定（OrganAMNIST、10 クライアント、40 ラウンド、α=0.5）では、AFAD の統合効果が精度差として十分に顕在化していない。以下の拡張により、有効性の検証を深める必要がある:

1. **Non-IID 強度の拡張**: α=0.1 等のより強い Non-IID 環境で、HeteroFL + FedGen の相補性が拡大するか検証
2. **スケール検証**: クライアント数を 20–50 に増加し、同族グループ内の集約効果が精度に寄与するか確認
3. **データセットの多様化**: CIFAR-10/100、PathMNIST 等の異なるタスクでの再現性検証
4. **長期的収束**: ラウンド数を 100 以上に拡張し、収束特性の差異を評価
5. **多角的評価**: 精度に加え、通信コスト、収束速度、クライアント間の公平性（各クライアントの精度分散）を評価指標に追加

### AFAD の意義

AFAD の主たる貢献は精度の絶対的向上ではなく、**異種アーキテクチャが混在する現実的な FL 環境において、同族間集約（HeteroFL）と異種間知識転写（FedGen）を統一的に扱える汎用フレームワークを提示した点**にある。HeteroFL 単体では CNN ↔ ViT 間の知識共有ができず、FedGen 単体では同族間の効率的な重み共有を活用できない。AFAD はこの両方を自動的に使い分けることで、モデル構成の制約なく連合学習を運用できる。
