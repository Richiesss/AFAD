"""HeteroFL-only client for baseline comparison.

Standard local training with SGD + optional FedProx. Supports HeteroFL
shape-aware parameter handling for width-scaled sub-models.

No knowledge distillation — this is the HeteroFL Only baseline.
"""
...
logger = setup_logger("HeteroFLClient")


class HeteroFLClient(fl.client.NumPyClient):
    """HeteroFL client with width-scaled sub-model support.

    Standard local training with SGD. No knowledge distillation.
    Used as the HeteroFL Only baseline in 3-way comparison.
    """
    ...
    def _train(
        ...
        """Local training with SGD, gradient clipping, and optional FedProx."""
